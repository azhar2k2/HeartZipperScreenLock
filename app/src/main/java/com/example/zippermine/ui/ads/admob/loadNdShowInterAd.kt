...
object loadNdShowINterAd {
    ...
    fun showInterstitial(activity: Activity, callback: InterstitialCallBack? = null) {
        if (interAdmob == null) {
            // Load the ad and set up the callback to show it once loaded
            loadInterAdmob(activity, callback = object : LoadAdCallBack {
    ...
    private fun showLoadedAd(activity: Activity, callback: InterstitialCallBack? = null) {
